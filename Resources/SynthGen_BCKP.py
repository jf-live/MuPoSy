...
class SynthGen:
    def __init__(self, freq=400, mod=1, mult=0.2, multMod=1):
        self.freq = freq
        self.mod = mod
        ...
        if vari.randEnvSynth >= 35:
            attackT = random.random()
            decayT = random.uniform(0.01,0.5)
            releaseT = random.random()
            dura = attackT + decayT + releaseT + 0.5
            self.env = Adsr(attackT, decayT, 0.7, releaseT, dura).play()
        else:
            self.env = 1.
    ...
    def setNewFreq(self,newFreq):
        ...
        if self.randChooser > 8 and self.randChooser <= 10:
            self.sig.setCarrier(self.freq)
        else:
            self.sig.setFreq(self.freq)
